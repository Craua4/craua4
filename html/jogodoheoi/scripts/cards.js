class CardManager {
    ...
    flip(cardNode) {
        cardNode.querySelector('.flip-card-inner').classList.add('selected');
    ...
    unFlip(cardNode) {
        cardNode.querySelector('.flip-card-inner').classList.remove('selected');
    }

    //Set received card as matched
    disable(cardNode) {
        cardNode.querySelector('.flip-card-inner').classList.add('matched')
    ...
    endTurn(){
        ...
        let handler = this.check() ? (card) => this.disable(card) : (card) => this.unFlip(card);
